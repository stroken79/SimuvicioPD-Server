...
local function updateHud(rank)
    ...
    nextRank = pointData.nextRank and pointData.nextRank.label or "Rango máximo",
    pointsLeft = pointData.nextRank and pointData.nextRank.remaining or 0,
    administrative = pointData.administrative
})
...
RegisterNetEvent('smvlpd-ems-ranks:client:showRank', function()
    ...
    notify(('Rango EMS: %s | Uniforme: %s'):format(rank.label, rank.uniform or 'Pendiente de configurar'))
end)
...
RegisterNetEvent('smvlpd-ems-ranks:client:openManagement', function(players)
    ...
    lib.registerContext({ id = 'smvlpd_rank_management', title = 'Gestión de rangos EMS', options = options })
...
RegisterNetEvent('smvlpd-ems-ranks:client:chooseRank', function(player)
    ...
    for rankId, rank in ipairs(Config.Ranks) do
        options[#options + 1] = {
            ...
            icon = rank.administrative and 'user-tie' or 'kit-medical',
