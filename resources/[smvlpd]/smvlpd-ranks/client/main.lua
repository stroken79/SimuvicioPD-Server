...
RegisterNetEvent('smvlpd-ranks:client:receiveWeapon', function(weapon)
    local ped = PlayerPedId()
    local weaponHash = joaat(weapon.name)
    GiveWeaponToPed(ped, weaponHash, weapon.ammo, false, false)
    for _, component in ipairs(weapon.components or {}) do
        GiveWeaponComponentToPed(ped, weaponHash, joaat(component))
    end
    notify(('Retirado: %s'):format(weapon.name:gsub('WEAPON_', ''):gsub('_', ' ')), 'success')
end)
