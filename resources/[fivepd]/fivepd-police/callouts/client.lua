...
RegisterNetEvent('fivepd-police:barfight:question')
AddEventHandler('fivepd-police:barfight:question', function(targetNetID)

    print(
        "^3[BARFIGHT QUESTION] Evento recibido. NetID: "
        .. tostring(targetNetID)
        .. "^7"
    )
    ...
    if not fighter1 or not fighter2 then
        print("^1[BARFIGHT QUESTION] fighter1 o fighter2 no existen.^7")
        return
    end

    if not DoesEntityExist(fighter1) or not DoesEntityExist(fighter2) then
        print("^1[BARFIGHT QUESTION] Uno de los implicados ya no existe.^7")
        return
    end

    local playerPed = PlayerPedId()

    local playerNetID = PedToNet(playerPed)
    local fighter1NetID = PedToNet(fighter1)
    local fighter2NetID = PedToNet(fighter2)

    print(
        "^3[BARFIGHT QUESTION] Objetivo: "
        .. tostring(targetNetID)
        .. " | Fighter1: "
    ...
    -- PRIMER IMPLICADO
    if targetNetID == fighter1NetID then
        ...
        TriggerServerEvent(
            ...
            targetNetID,
            playerNetID,
            'Ese tipo empezó a provocarme. Intenté apartarme, pero me atacó y tuve que defenderme.',
            5000
        )

        print("^2[BARFIGHT QUESTION] Respondió Fighter1.^7")

    -- SEGUNDO IMPLICADO
    elseif targetNetID == fighter2NetID then
        ...
        TriggerServerEvent(
            ...
            targetNetID,
            playerNetID,
            'Él empezó la pelea. Yo solo respondí cuando vino hacia mí. Pregúntele a él.',
            5000
        )

        print("^2[BARFIGHT QUESTION] Respondió Fighter2.^7")

    else

        print(
            "^1[BARFIGHT QUESTION] El peatón seleccionado no pertenece al callout.^7"
        )
